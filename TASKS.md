...
- 2025-10-24: OPS-B02（Permit 再評価フロー）を完了。`pytest tests/integration/test_runtime_multicontent_failures.py -k permit -q` → `mypy src/llm_generic_bot/core/orchestrator` → `ruff check src/llm_generic_bot/core/orchestrator` で監査ログ・メトリクス更新を固定し、再評価待ちキュー投入とレート制御の整合を確認。
- 2025-10-24: OPS-B16（メトリクス集計レコード分離）を完了。`pytest tests/infra/metrics/recording/test_success_events.py -q && pytest tests/infra/metrics/recording/test_delay_events.py -q && mypy src/llm_generic_bot/infra/metrics && ruff check src/llm_generic_bot/infra/metrics` を実行し、`infra/metrics/aggregator_state.py` と `aggregator_records.py` への責務分離と retention 境界テストを完了。
- 2025-10-24: OPS-B03（Permit クォータ多段構成と再送ガード）を完了。`pytest tests/core/test_quota_gate.py -k multilayer -q && pytest tests/integration/test_runtime_multicontent_failures.py -k quota_multilayer -q && mypy src/llm_generic_bot/core/queue.py && ruff check src/llm_generic_bot/core/queue.py` を通過し、`CoalesceQueue` と `Scheduler` の再送ガード調整を確定。
...
- 2025-10-24: OPS-B01/OPS-B02/OPS-B03/OPS-B16 の実装・テスト完了を確認し、`pytest`・`mypy`・`ruff` コマンド群の緑化結果を docs/tasks/backlog.md・docs/roadmap.md へ反映済み。
...
- 2025-10-24: `tests/core/quota_gate/test_multitier.py` と `tests/integration/test_runtime_multicontent_failures.py` に多段ティア再送ガード検証を追加し、`CoalesceQueue.pending_reevaluation_levels` と `Scheduler.mark_reevaluation_pending` でティア別再評価待機と `batch_id` 再利用抑止を共有。`pytest tests/core/quota_gate -k mult -q` → `pytest tests/integration/test_runtime_multicontent_failures.py -k quota_multilayer -q` → `mypy src/llm_generic_bot/core/queue.py` → `ruff check src/llm_generic_bot/core/queue.py` を連続実行し、queue/scheduler では複数タスクが並走している旨を継続共有する。

- 2025-10-24: `tests/integration/test_runtime_multicontent_failures.py`・`tests/infra/metrics/test_send_delay_thresholds.py`・`src/llm_generic_bot/core/scheduler.py`・`config/settings.example.json` でジッタ閾値とバッチ閾値の揺らぎ検証/設定反映が並走するため、本行で共有する。`pytest tests/integration/test_runtime_multicontent_failures.py -q && pytest tests/infra/metrics/test_send_delay_thresholds.py -q && mypy src/llm_generic_bot/core/scheduler.py src/llm_generic_bot/core/arbiter.py && ruff check src/llm_generic_bot/core/scheduler.py src/llm_generic_bot/core/arbiter.py` を緑化した上で差分を適用すること。
- 2025-10-22: tests/integration/test_runtime_multicontent_failures.py と src/llm_generic_bot/core/orchestrator/{processor.py,orchestrator_metrics.py}・src/llm_generic_bot/core/arbiter.py で Permit 再評価待ちキュー投入と再送メトリクス記録を強化。`pytest tests/integration/test_runtime_multicontent_failures.py -k permit -q` → `mypy src/llm_generic_bot/core/orchestrator` → `ruff check src/llm_generic_bot/core/orchestrator` を順に実行する複数タスクが並行している旨を共有する。
- 2025-10-24: tests/infra/metrics/recording/test_success_events.py・test_delay_events.py の retention 境界テストと `infra/metrics/aggregator_state.py`→`aggregator_records.py` の純粋ロジック移管を並走させる。`pytest tests/infra/metrics/recording/test_success_events.py -q && pytest tests/infra/metrics/recording/test_delay_events.py -q && mypy src/llm_generic_bot/infra/metrics && ruff check src/llm_generic_bot/infra/metrics` を連続実行してから差分適用することを共有。
- 2025-10-21: docs/roadmap.md の OPS 残課題節へ `infra.metrics.reporting.report_send_delay` が `send.delay_seconds` を `unit="seconds"` タグ付きで記録する旨を追記し、`pytest tests/infra/metrics/recording/test_delay_events.py -k report_send_delay_records_unit_seconds -q` と `npx markdownlint-cli docs/tasks/backlog.md` を実行して記述整合と整形を確認した。docs/tasks/backlog.md の DOC-B15 行備考へ同内容の再確認を追記。
- 2025-10-23: docs/roadmap.md の 残課題 OPS 節へ [OPS-B16] の要約と対象モジュール（`infra/metrics/aggregator_state.py`・`infra/metrics/aggregator_records.py`）および現行テスト（`tests/infra/metrics/recording/test_success_events.py`・`tests/infra/metrics/recording/test_delay_events.py`）を追記し、`npx markdownlint-cli docs/roadmap.md` で整形を確認する作業を開始。docs/roadmap.md ではタスクが並行しているため、重複管理情報を共有する。
- 2025-10-21: docs/roadmap.md の 残課題 OPS 節へ [OPS-B16] の要約と対象モジュール（`infra/metrics/aggregator_state.py`・`infra/metrics/aggregator_records.py`）および現行テスト（`tests/infra/metrics/recording/test_success_events.py`・`tests/infra/metrics/recording/test_delay_events.py`）を追記し、`npx markdownlint-cli docs/roadmap.md` で整形を確認するタスクへ着手。docs/roadmap.md には並行タスクが増加しているため、重複管理の共有を継続する。
- 2025-10-21: docs/roadmap.md の OPS 残課題節へ `report_send_delay` 呼び出し由来の `send.delay_seconds` 秒単位タグ説明を補足し、`pytest tests/infra/metrics/recording/test_delay_events.py -k report_send_delay_records_unit_seconds -q` 実行結果と `npx markdownlint-cli docs/tasks/backlog.md` による整形確認、docs/tasks/backlog.md の DOC-B15 行完了化を記録する。
- 2025-10-21: `tests/core/quota_gate/test_multitier.py` と `tests/integration/test_runtime_multicontent_failures.py` へ多段ティア再送ガード検証を追加し、`CoalesceQueue.mark_reevaluation_pending` でティア別再評価待機と `batch_id` 再利用抑止を共有。`pytest tests/core/quota_gate -k mult -q` → `pytest tests/integration/test_runtime_multicontent_failures.py -k quota_multilayer -q` → `mypy src/llm_generic_bot/core/queue.py` → `ruff check src/llm_generic_bot/core/queue.py` を連続実行し、queue/scheduler では複数タスクが並走している旨を継続共有する。
- 2025-10-21: src/llm_generic_bot/core/queue.py と src/llm_generic_bot/core/scheduler.py でバッチ再送ガード（ティア別再評価待ちと `batch_id` 重複抑止）を実装。`pytest tests/core/quota_gate -k multilayer -q` → `pytest tests/integration/test_runtime_multicontent_failures.py -k quota_multilayer -q` → `mypy src/llm_generic_bot/core/queue.py` → `ruff check src/llm_generic_bot/core/queue.py` を実行済みで、同ファイルの並行タスクが発生している旨を共有する。
- 2025-10-23: `tests/core/quota_gate/test_multitier.py` と `tests/integration/test_runtime_multicontent_failures.py` へ多段ティア再送ガード検証を追加し、`CoalesceQueue.mark_reevaluation_pending` でティア別再評価待機と `batch_id` 再利用抑止を共有。`pytest tests/core/quota_gate -k mult -q` → `pytest tests/integration/test_runtime_multicontent_failures.py -k quota_multilayer -q` → `mypy src/llm_generic_bot/core/queue.py` → `ruff check src/llm_generic_bot/core/queue.py` を連続実行し、queue/scheduler では複数タスクが並走している旨を継続共有する。
- 2025-10-22: src/llm_generic_bot/core/queue.py と src/llm_generic_bot/core/scheduler.py でバッチ再送ガード（ティア別再評価待ちと `batch_id` 重複抑止）を実装。`pytest tests/core/quota_gate -k multilayer -q` → `pytest tests/integration/test_runtime_multicontent_failures.py -k quota_multilayer -q` → `mypy src/llm_generic_bot/core/queue.py` → `ruff check src/llm_generic_bot/core/queue.py` を実行済みで、同ファイルの並行タスクが発生している旨を共有する。
- 2025-10-21: `tests/integration/test_runtime_multicontent_failures.py`・`tests/infra/metrics/test_send_delay_thresholds.py` のジッタ閾値揺らぎ検証を追加し、同件で `src/llm_generic_bot/core/scheduler.py` と `config/settings.example.json` も並行タスク管理対象になったことを共有する。`pytest tests/integration/test_runtime_multicontent_failures.py -q && pytest tests/infra/metrics/test_send_delay_thresholds.py -q && mypy src/llm_generic_bot/core/scheduler.py src/llm_generic_bot/core/arbiter.py && ruff check src/llm_generic_bot/core/scheduler.py src/llm_generic_bot/core/arbiter.py` を先に実行してから差分を適用すること。
- 2025-10-21: docs/roadmap.md の 残課題 OPS 節へ `send.delay_seconds` の `unit="seconds"` タグ計測と検証コマンド（`pytest tests/infra/metrics/recording/test_delay_events.py -k report_send_delay_records_unit_seconds -q`）を追記し、`npx markdownlint-cli docs/roadmap.md` で整形確認した。同ファイルでは複数タスクが並行している旨を引き続き共有する。
- 2025-10-21: tests/infra/metrics/recording/test_success_events.py と test_delay_events.py へ履歴トリム検証を追加し、aggregator_state から aggregator_records への純粋ロジック移管を並行実施した。`pytest tests/infra/metrics/recording/test_success_events.py -q` → `pytest tests/infra/metrics/recording/test_delay_events.py -q` → `mypy src/llm_generic_bot/infra/metrics` → `ruff check src/llm_generic_bot/infra/metrics` を連続実行する複数タスクが同時進行中である旨を共有する。
- 2025-10-21: `tests/integration/test_runtime_multicontent_failures.py` に Permit 再評価再送テストを追加し、同ファイルに複数タスクが併走している旨を共有。`pytest tests/integration/test_runtime_multicontent_failures.py -k permit -q` を実行し、再送理由ログとメトリクスの確認を完了してからチェックリストを更新すること。
- 2025-10-21: src/llm_generic_bot/core/scheduler.py でジッタ許容遅延とバッチ閾値計測タスクが並行稼働するため、本行で重複管理を共有しつつ `pytest tests/integration/test_runtime_multicontent_failures.py -q && pytest tests/infra/metrics/test_send_delay_thresholds.py -q` を確認する。
- 2025-10-21: docs/tasks/backlog.md の UX-B01 行に分割後テスト関数（`tests/features/weather_engagement/test_scoring.py::test_weather_engagement_long_term_trend_blends_recent_history`・`::test_weather_engagement_trend_respects_permit_quota_variation`・`tests/features/weather_engagement/test_resume_conditions.py::test_weather_engagement_resume_thresholds`・`tests/features/weather_engagement/test_cooldown.py::test_send_success_log_contains_engagement`）を対象領域へ明記し、完了条件を各 `pytest tests/features/weather_engagement/... -q` コマンドへ更新した。`npx markdownlint-cli docs/tasks/backlog.md` を実行し、同ファイルで複数タスクが併走している旨を継続共有する。
- 2025-10-21: docs/roadmap.md Sprint2 UX Weather Engagement 節を `tests/features/weather_engagement/test_cooldown.py`・`test_resume_conditions.py`・`test_scoring.py` の現行ケース参照へ揃え、旧シム名を撤去した。`npx markdownlint-cli docs/roadmap.md` を実行済みで、同ファイルには並行タスクが継続している旨を共有する。
- 2025-10-20: docs/tasks/backlog.md の UX-B01 行を `tests/features/weather_engagement/test_scoring.py::test_weather_engagement_long_term_trend_blends_recent_history` ほか現行ケースへ差し替え、`tests/features/weather_engagement/test_resume_conditions.py::test_weather_engagement_resume_thresholds` も参照に追加した。`npx markdownlint-cli docs/tasks/backlog.md` を実行し、同ファイルに複数タスクが併存している旨を改めて共有する。
- 2025-10-20: docs/roadmap.md の UX-01/UX-05/UX-B01 節を Weather Engagement 新構成テスト（`tests/features/weather_engagement/test_cooldown.py`・`test_resume_conditions.py`・`test_scoring.py`）参照へ更新し、ケース名を現行実装に揃えた。`npx markdownlint-cli docs/roadmap.md` を実行済みで、同ファイルに並行タスクが継続している旨を共有する。
- 2025-10-20: docs/roadmap.md OPS-10 節の Permit 拒否テスト参照を `tests/integration/runtime_multicontent/failures/test_permit.py::test_permit_denied_records_metrics` へ差し替え、同段落内の残り結合テスト参照と整合させた。`npx markdownlint-cli docs/roadmap.md` を実行済みで、同ファイルでは複数タスクが並行中である旨を共有する.
- 2025-10-21: docs/tasks/sprint2.md の UX-01 行で確認テスト欄を `tests/features/weather_engagement/` 配下の分割ファイル群へ差し替え、クールダウン・再開条件・スコアリング・履歴フィルタの各検証タスクが並行中であることを明示した。`npx markdownlint-cli docs/tasks/sprint2.md` による表整形確認済みで、Sprint2 ドキュメントへ新たな並行タスクが加わった旨を共有する。
- 2025-10-20: docs/roadmap.md OPS-10 節の Permit 拒否テスト参照を `tests/integration/runtime_multicontent/failures/test_permit.py::test_permit_denied_records_metrics` へ差し替え、同段落内の残り結合テスト参照と整合させた。`npx markdownlint-cli docs/roadmap.md` を実行済みで、同ファイルでは複数タスクが並行中である旨を共有する。
- 2025-10-20: docs/roadmap.md Sprint3 テストロードマップ節を `tests/infra/metrics/recording/test_failures.py` 参照へ更新し、`send.failure`・`send.duration` ラベル固定化の説明を追加。`npx markdownlint-cli docs/roadmap.md` を実行し、同ファイルでは複数タスクが並行中である旨を継続共有する。
...
- 2025-10-20: WEATHER-LEGACY-01 を登録。`tests/features/weather_engagement/test_scoring.py::test_weather_engagement_long_term_trend_blends_recent_history` と `tests/features/weather_engagement/test_resume_conditions.py::test_weather_engagement_resume_thresholds` など分割済みケースを恒常運用とし、`LEGACY_WEATHER_ENGAGEMENT_TEST_CHECKLIST` の全項目完了後は `pytest tests/features/weather_engagement/test_scoring.py::test_weather_engagement_long_term_trend_blends_recent_history -q` および `pytest tests/features/weather_engagement/test_resume_conditions.py::test_weather_engagement_resume_thresholds -q` 緑化を確認する段取りを追記（補足: 旧レガシーシム `tests/features/test_weather_engagement.py`）。
...
- 2025-10-20: OPS-B16・UX-B01・DOC-B15 の参照を `tests/infra/metrics/recording/` 新テスト群へ差し替え、`pytest tests/infra/metrics/recording/test_success_events.py -q`・`pytest tests/infra/metrics/recording/test_delay_events.py -q` を指標とした更新タスクを追加。docs/tasks/backlog.md には同一ファイル内で複数タスクが並列している旨を明示し続ける。
...
- 2025-10-19: docs/tasks/sprint1.md の SND-02 / OPS-01 で旧 `core/orchestrator.py` 表記を `core/orchestrator/__init__.py`（移行先）と `core/orchestrator/processor.py`（送信処理ロジック）へ差し替え、同一ファイルに複数タスク（SND-02・OPS-01）が積まれていることを明示。`markdownlint docs/tasks/sprint1.md` は `command not found` を確認し、`git diff docs/tasks/sprint1.md` で差分を確認。
- 2025-10-20: docs/tasks/sprint2.md の UX-01 対象モジュール列を `core/orchestrator/__init__.py`（必要時 `core/orchestrator/processor.py`）へ改め、旧 `core/orchestrator.py` 参照を移行済みであることを追記。`markdownlint docs/tasks/sprint2.md` は `command not found` を確認し、`git diff docs/tasks/sprint2.md` で差分を再確認。Sprint2 ドキュメントに複数タスクが併記されている旨も引き続き記録。
- 2025-10-19: docs/tasks/sprint3.md の OPS-02 / OPS-04 行で `src/llm_generic_bot/core/orchestrator/__init__.py`・`src/llm_generic_bot/core/orchestrator/processor.py` 表記へ更新し、`markdownlint docs/tasks/sprint3.md`（command not found を再確認）と `git diff docs/tasks/sprint3.md` を実行して差分を確認。Sprint3 リストには OPS-02・OPS-03・OPS-04・OPS-07 など複数タスクが併記されている旨を備考で再度明示。
- 2025-10-19: docs/tasks/backlog.md の OPS-B02 / UX-B01 行を `core/orchestrator/__init__.py`・`core/orchestrator/processor.py` 参照強調へ再調整し、同ファイルの重複更新として `markdownlint docs/tasks/backlog.md`（command not found）と `git diff docs/tasks/backlog.md` を実行して差分確認を再記録。
- 2025-10-19: docs/tasks/sprint1.md の SND-02 / OPS-01 で対象モジュールを `core/orchestrator/__init__.py`（エントリポイント）へ明示し、同一ファイルに複数タスクが積まれている旨を備考へ残した。`markdownlint docs/tasks/sprint1.md` は `command not found` を確認し、`git diff docs/tasks/sprint1.md` で差分を確認。
...
- 2025-10-19: docs/tasks/sprint3.md の OPS-02/OPS-04 説明を `core/orchestrator/__init__.py`・`core/orchestrator/processor.py` 参照へ更新し、`markdownlint docs/tasks/sprint3.md`（command not found を確認）と `git diff docs/tasks/sprint3.md` を実行。Sprint3 タスクリストに複数タスクが併記されている旨を再確認して記録。
...
- 2025-10-19: docs/roadmap.md の `core/orchestrator/__init__.py`／`core/orchestrator/processor.py` 記述を ` と ` 区切りへ再補正し、旧 `core/orchestrator.py`（削除済み）注記も併記して構成整合を維持。2025-10-19 記録済みタスクの追従作業として `markdownlint docs/roadmap.md`（環境未導入で失敗）と `git diff docs/roadmap.md` を再実行して差分を確認。
...
- 2025-10-18: UX-B01 Engagement 長期トレンド分析タスクをスタブ化。`tests/features/weather_engagement/test_scoring.py::test_weather_engagement_long_term_trend_blends_recent_history` と `tests/features/weather_engagement/test_scoring.py::test_weather_engagement_trend_respects_permit_quota_variation` を先に実装し、`pytest tests/features/weather_engagement/test_scoring.py::test_weather_engagement_long_term_trend_blends_recent_history -q` と `pytest tests/features/weather_engagement/test_scoring.py::test_weather_engagement_trend_respects_permit_quota_variation -q` 緑化を確認した上で Permit クォータ連動ロジックの調整タスクを設定。

- 2025-10-21: OPS-B06 追補として weather 機能をモジュール分割し、`pytest tests/features/test_weather_cache_rotation.py tests/features/test_weather_engagement.py -q` → `pytest tests/integration/weather_engagement -q` → `mypy src/llm_generic_bot/features/weather.py src/llm_generic_bot/features/weather` → `ruff check src/llm_generic_bot/features/weather` を順に緑化した。

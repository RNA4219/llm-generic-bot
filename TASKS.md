- 2025-10-21: runtime_multicontent failures 再編タスクの移行手順を追記。`tests/integration/test_runtime_multicontent_failures.py` の `LEGACY_MULTICONTENT_FAILURES_CHECKLIST` に新モジュールを列挙しつつ、各ケースの移行時は `pytest tests/integration/runtime_multicontent/failures -k <scenario> -q` を緑化確認してからチェックリスト項目を削除すること。

...
- 2025-10-20: WEATHER-LEGACY-01 を登録。`tests/features/weather_engagement/test_scoring.py` ほか分割済みテストを恒常運用とし、`LEGACY_WEATHER_ENGAGEMENT_TEST_CHECKLIST` の全項目完了後に `pytest tests/features/weather_engagement -q` 緑化を確認する段取りを追記（注記: 旧レガシーシム `tests/features/test_weather_engagement.py`）。
...
- 2025-10-18: UX-B01 Engagement 長期トレンド分析タスクをスタブ化。`tests/features/weather_engagement/test_scoring.py::test_weather_engagement_long_term_trend_blends_recent_history` と `::test_weather_engagement_trend_respects_permit_quota_variation` を先に実装し、`pytest tests/features/weather_engagement -q` 緑化を確認した上で Permit クォータ連動ロジックの調整タスクを設定。

- 2025-10-22: docs/tasks/sprint2.md の UX-01 対象モジュール列を `core/orchestrator/__init__.py`（必要時 `core/orchestrator/processor.py`）へ改め、旧 `core/orchestrator.py` 参照を移行済みであることを追記。`markdownlint docs/tasks/sprint2.md` は `command not found` を確認し、`git diff docs/tasks/sprint2.md` で差分を再確認。Sprint2 ドキュメントに複数タスクが併記されている旨も引き続き記録。

- 2025-10-22: `tests/infra/metrics/recording/test_success_events.py`・`test_delay_events.py` へ未来時刻トリム境界の再追加を行い、`src/llm_generic_bot/infra/metrics/aggregator_state.py` のロック責務単純化と `aggregator_records.py` へのロジック移管を並行で進行中。`pytest tests/infra/metrics/recording/test_success_events.py -q` → `pytest tests/infra/metrics/recording/test_delay_events.py -q` → `mypy src/llm_generic_bot/infra/metrics` → `ruff check src/llm_generic_bot/infra/metrics` を順に実行しつつ、同領域で複数タスクが同時発生している旨を共有する。

...
- `send.delay_seconds` は `tests/infra/metrics/recording/test_delay_events.py::test_report_send_delay_records_unit_seconds` で `reporting.report_send_delay(..., delay_seconds=...)` 呼び出しから `unit="seconds"` タグ付き計測を固定化し、`pytest tests/infra/metrics/recording/test_delay_events.py -k report_send_delay_records_unit_seconds -q` で検証する。

...
- [OPS-B01] Permit/ジッタ/バッチ閾値の運用チューニング: `pytest tests/integration/test_runtime_multicontent_failures.py -q`・`pytest tests/infra/metrics/test_send_delay_thresholds.py -q`・`mypy src/llm_generic_bot/core/scheduler.py src/llm_generic_bot/core/arbiter.py`・`ruff check src/llm_generic_bot/core/scheduler.py src/llm_generic_bot/core/arbiter.py` を完走し、`config/settings.example.json` のジッタ・バッチ閾値調整と遅延メトリクス揺らぎの安定化を確認（2025-10-24 完了）。
- [OPS-B02] Permit 失敗時の再評価フロー整備: `pytest tests/integration/test_runtime_multicontent_failures.py -k permit -q`・`mypy src/llm_generic_bot/core/orchestrator`・`ruff check src/llm_generic_bot/core/orchestrator` で再評価待ちキュー投入、監査ログ、再送メトリクス記録を固定し、PermitGate レート制御と重複抑止を両立（2025-10-24 完了）。
- [OPS-B03] Permit クォータ多段構成とバッチ再送ガード: `pytest tests/core/test_quota_gate.py -k multilayer -q`・`pytest tests/integration/test_runtime_multicontent_failures.py -k quota_multilayer -q`・`mypy src/llm_generic_bot/core/queue.py`・`ruff check src/llm_generic_bot/core/queue.py` を通過し、`CoalesceQueue` と `Scheduler` のティア別再送ガードと `batch_id` 再利用抑止を確定（2025-10-24 完了）。
...
### 残課題
- [OPS-B01/B02/B03] Permit 系 3 タスクはバックログ（`docs/tasks/backlog.md`）を基準に継続中。ジッタ・バッチ閾値調整時は `tests/integration/test_runtime_multicontent_failures.py` を先行実行して回帰を確認し、Permit 再評価フローと多段クォータ再送ガードも `tests/core/test_quota_gate.py` 拡張と併せて検証を追加する。
- [OPS-B16] メトリクス履歴集計の状態管理と記録経路を `infra/metrics/aggregator_state.py`・`infra/metrics/aggregator_records.py` へ整理し、`tests/infra/metrics/recording/test_success_events.py`・`tests/infra/metrics/recording/test_delay_events.py` で回帰を固定するタスクが継続中。
...
#### OPS（運用・基盤）
- 現在、OPS 系の残課題はなし。2025-10-24 に OPS-B01/OPS-B02/OPS-B03/OPS-B16 を完了し、前述のテスト・静的解析コマンドで回帰確認済み。
...
- [OPS-04] ランタイムメトリクス（`src/llm_generic_bot/infra/metrics/aggregator.py`, `src/llm_generic_bot/infra/metrics/aggregator_state.py`, `src/llm_generic_bot/infra/metrics/reporting.py`, `src/llm_generic_bot/infra/metrics/service.py`）: `aggregator.py` が送信/Permit 事象の公開ファサードとなり、`aggregator_state.py` がロック付きの履歴保持と週次スナップショット生成を担いつつ、`service.py` のバックエンド構成と `reporting.py` の集約ロジックへ橋渡しする。
  - `tests/infra/metrics/recording/test_delay_events.py::test_report_send_delay_records_unit_seconds`: `infra.metrics.reporting.report_send_delay` が `send.delay_seconds` を `unit=seconds` タグ付きで記録することを固定する（検証: `pytest tests/infra/metrics/recording/test_delay_events.py -k report_send_delay_records_unit_seconds -q`）。
...
- Sprint 3: ランタイムメトリクスの結合テストは `tests/infra/metrics/recording/` 配下モジュール（`test_success_events.py`・`test_delay_events.py`・`test_failures.py` ほか）で記録タグ境界を網羅し、失敗計測は `test_failures.py` が `send.failure` と `send.duration` のラベル（`unit="seconds"` 含む）を固定化する。`tests/infra/metrics/test_reporting_freeze_time.py` と `tests/infra/metrics/test_reporting_service.py` がレポーティング API のフリーズ時間制御とサービス層の統合挙動を検証する構成へ再編済み。これらの分割テストを前提に `tests/core/structured_logging/test_metrics.py` を拡張し、`MetricsRecorder.observe` 呼び出しの単位検証を追加する。

...
- integration テストは以下で運用経路をカバーしている:
  ...
  - `tests/integration/runtime_multicontent/test_pipeline_dm_digest.py`: DM ダイジェストジョブがスケジューラへ登録された後もキュー push や dispatch が発生しないことを保証。
...
- **DOC（ドキュメント）**
  ...
  - [DOC-B10] デデュープ無効化テストと `cooldown.jobs` 検証内容のロードマップ反映は完了済みで、該当テスト（`tests/runtime/test_setup_runtime_dedupe.py` と `tests/config/test_settings_example_cooldown.py`）を完了条件として記録済み。

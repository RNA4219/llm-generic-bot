...
| [x] | SND-02 | Permit ゲートでチャンネル別クォータ制御 | `src/llm_generic_bot/core/arbiter.py`<br>`src/llm_generic_bot/config/quotas.py`<br>`src/llm_generic_bot/core/orchestrator/__init__.py`（Orchestrator エントリポイント／旧 `core/orchestrator.py` の移行先）<br>`src/llm_generic_bot/core/orchestrator/processor.py`（送信処理ロジック） | `main.setup_runtime` で PermitGate をラップした構成を Orchestrator が利用し、バッチ送信時もクォータ超過を抑止して拒否理由をメトリクス/ログへ出力する。送信判定ロジックは `core/orchestrator/processor.py` へ移設済み。 | 残課題: クォータ閾値の運用チューニングと監視ダッシュボードへの反映。 | `tests/core/test_quota_gate.py`: 上限到達・リセット・許可ケース<br>`tests/integration/test_main_pipeline.py`: オーケストレータ経由バッチで PermitGate がクォータ制御する正常系<br>`tests/integration/test_permit_bridge.py`: PermitGate と Orchestrator の連携で拒否理由伝搬とメトリクス更新を検証 |
...
| [x] | OPS-01 | 送信処理の構造化ログ出力 | `src/llm_generic_bot/adapters/*.py`<br>`src/llm_generic_bot/core/orchestrator/__init__.py`（Orchestrator エントリポイント／旧 `core/orchestrator.py` の移行先）<br>`src/llm_generic_bot/core/orchestrator/processor.py`（送信結果集約） | 成功/失敗イベントを JSON 形式で出力し、Correlation ID を常に付与する。送信結果の構造化ログ生成は `core/orchestrator/processor.py` に集約済み。 | 追加要望: 可観測性チームとフォーマット拡張を検討中。 | `tests/core/test_structured_logging.py`: ログフォーマット・エラー経路 |

...
async def test_scheduler_records_delay_threshold_metrics(
    ...
    expected_min = (
        ...
        {"job": "news", "channel": "dispatch", "platform": "discord", "bound": "min"},
    )
    expected_max = (
        "send.delay_threshold_seconds",
        float(effective_range[1]),
        {"job": "news", "channel": "dispatch", "platform": "discord", "bound": "max"},
    )
    expected_threshold = (
        "send.batch_threshold_count",
        float(threshold),
        {"job": "news", "channel": "dispatch", "platform": "discord"},
    )
...
@pytest.mark.anyio("asyncio")
async def test_scheduler_delay_threshold_metrics_include_platform_tag(
    make_recording_metrics: Callable[[], MetricsRecorder],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    recorder = cast(RecordingMetricsLike, make_recording_metrics())
    ...
    assert {call[2].get("platform") for call in delay_threshold_calls} == {"discord"}
    assert {call[2].get("platform") for call in batch_threshold_calls} == {"discord"}
